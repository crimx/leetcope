...
But take a closer look and we realize that only one the two halves needs to be sorted. This is sufficient for us to know if the target is in that half. If not, then it must be in the other.
...
- If the pivot is in the first part. We can use the first half of the first part to locate the target.
- Otherwise the pivot is in the second part. We can use the second half of the second part to locate the target.
